...
export default function HistoryPage() {
  ...
  const { playSong, setQueue, currentIndex, queue, isPlaying, togglePlay, favorites, toggleFavorite, history: histList, playlists, addSongToPlaylist } = usePlayerStore()
  ...
  useEffect(() => {
    ...
    const ordered = histList.map(h => songs.find(s => s.id === h)).filter(Boolean) as Song[]

    setFiltered(ordered.filter(s => {
      if (!q) return true
      return s.title.toLowerCase().includes(q) ||
        s.artist.toLowerCase().includes(q) ||
        (s.album || '').toLowerCase().includes(q)
    }))
  }, [search, songs, histList])
        ...
        <div className="lib-actions-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px' }}>
              <h1 className="lib-title" style={{ fontSize: '28px', fontWeight: 700, letterSpacing: '-0.5px' }}>Ouvidas Recentemente</h1>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 24px' }}>
            ...
            <h3 style={{ fontSize: '18px', fontWeight: 600, marginBottom: '8px' }}>
              {search ? 'Nenhum resultado encontrado' : 'Nenhuma música tocada recentemente'}
            </h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '24px' }}>
              {search ? 'Tente buscar por outro termo' : 'O que você ouvir no player aparecerá aqui.'}
            </p>
          </div>
        ) : (
          <>
